...
def prepare_match_features(player_stats_df):
    """
    Prepare match features for random forest.
...
def train_and_evaluate(X_train, y_train, X_test, y_test, feature_names):
    ...
    # Using more trees for better performance, but can be tuned
...
def save_model(results, output_dir='output'):
    ...
    model_path = os.path.join(output_dir, 'esports_prediction_rf_model.pkl')
...
def main(file_path, save_model_file=True):
    ...
    print("\nFeature Importance:")
    for feature, importance in sorted(results_old['feature_importance'].items(), key=lambda x: x[1], reverse=True):
    ...
    print("\nFeature Importance:")
    for feature, importance in sorted(results_new['feature_importance'].items(), key=lambda x: x[1], reverse=True):
        print(f"  {feature}: {importance:.4f}")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description='Train Random Forest esports prediction model')
